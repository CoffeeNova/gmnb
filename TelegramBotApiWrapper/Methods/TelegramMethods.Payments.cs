...
namespace CoffeeJelly.TelegramBotApiWrapper.Methods
{
    public partial class TelegramMethods
    {
        ...
        public InvoiceMessage SendInvoice(string chatId, string title, string description, string payload, string providerToken,
            ...
            bool disableNotification = false, int? replyToMessageId = null, IMarkup replyMarkup = null)
        {
            ...
            var parameters = new NameValueCollection
            {
                ...
                {"currency", currency},
                {"prices", JsonConvert.SerializeObject(prices, Formatting.None, Settings)}
            };

            if (photoUrl != null)
                parameters.Add("photo_url", photoUrl);
            if (photoSize != 0)
                parameters.Add("photo_size", photoSize.ToString());
            if (photoWidth != 0)
                parameters.Add("photo_width", photoWidth.ToString());
            if (photoHeight != 0)
                parameters.Add("photo_height", photoHeight.ToString());
            if (needName)
                parameters.Add("need_name", true.ToString());
            if (needPhoneNumber)
                parameters.Add("need_phone_number", true.ToString());
            if (needEmail)
                parameters.Add("need_email", true.ToString());
            if (needShipingAddress)
                parameters.Add("need_shiping_address", true.ToString());
            if (isFlexible)
                parameters.Add("is_flexible", true.ToString());
            ...
            return MessageBuilder.BuildMessage<InvoiceMessage>(json["result"]);
        }

        /// <summary>
        /// Use this method to send invoices asynchronously.
        /// </summary>
        /// <param name="chatId">Unique identifier for the target private chat.</param>
        /// <param name="title">Product name.</param>
        /// <param name="description">Product description.</param>
        ...
        public Task<InvoiceMessage> SendInvoiceAsync(string chatId, string title, string description, string payload,
            string providerToken, string startParameter, string currency, List<LabeledPrice> prices, string photoUrl = null,
            int photoSize = 0, int photoWidth = 0, int photoHeight = 0, bool needName = false, bool needPhoneNumber = false,
            bool needEmail = false, bool needShipingAddress = false, bool isFlexible = false,
            bool disableNotification = false, int? replyToMessageId = null, IMarkup replyMarkup = null)
        {
            return Task.Run(() => SendInvoice(chatId, title, description, payload, providerToken, startParameter, currency, prices,
                photoUrl, photoSize, photoWidth, photoHeight, needName, needPhoneNumber, needEmail, needShipingAddress,
                isFlexible, disableNotification, replyToMessageId, replyMarkup));
        }
        ...
        public bool AnswerShippingQuery(string shippingQueryId, bool ok, List<ShippingOption> shippingOptions = null,
            string errorMessage = null)
        {
            ...
            if (shippingOptions != null)
                parameters.Add("shipping_options", JsonConvert.SerializeObject(shippingOptions, Formatting.None, Settings));
        ...
        /// <summary>
        /// Async implementation of <see cref="AnswerShippingQuery"/> method. 
        /// If you sent an invoice requesting a shipping address and the parameter <see langword="isFlexible"/> was specified, 
        /// the Bot API will send an <see cref="Update"/> with a <see langword="shipping_query"/> field to the bot. Use this method to reply to shipping queries. 
        /// On success, <see langword="true"/> is returned.
        /// </summary>
        ...
        /// Telegram will display this message to the user.</param>
        /// <returns><see langword="true"/> on success.</returns>
        public Task<bool> AnswerShippingQueryAsync(string shippingQueryId, bool ok, List<ShippingOption> shippingOptions = null,
            string errorMessage = null)
        {
            return Task.Run(() => AnswerShippingQuery(shippingQueryId, ok, shippingOptions, errorMessage));
        }
        ...
        /// <summary>
        /// Async implementation of <see cref="AnswerPreCheckoutQuery"/> method. 
        /// Once the user has confirmed their payment and shipping details, the Bot API sends the final confirmation in the form of an <see cref="Update"/> 
        /// with the <see langword="pre_checkout_query"/> field . 
        /// Use this method to respond to such pre-checkout queries.
        /// </summary>
        ...
        {
            return Task.Run(() => AnswerPreCheckoutQuery(preCheckoutQueryId, ok, errorMessage));
        }
    }
}
